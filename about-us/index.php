<?php include("../includes/var.php") ?>
<!DOCTYPE html>
<html lang="en">
    <head>
        <title>Home | Binaro</title>
        <?php include("../includes/compatibility.php") ?>
        <?php include("../includes/styles.php") ?>
    </head>
    <body>
        <?php include("../includes/header.php") ?>
        <section id="abt-banner-section">
            <div class="abt-bg">
                <div class="container">
                    <div class="row justify-content-center">
                        <div class="col-lg-6 col-md-6 col-md-12">
                            <div class="heading-style03-img">
                                <img src="assets\images\icons\Vector.png" class="hd-styimg-03 img-fluid" />
                            </div>
                            <div class="abt-text-block">
                                <h2>About Binaro</h2>
                                <p>
                                    Binaro Group is your precise, customized, and economical partner for Design, Development, and Media Services. We are more than just a service provider; we believe in thinking big, being innovative, and
                                    embracing challenges to develop smart and customized solutions that solve problems in unexpected ways. Discover how Binaro Group can help you achieve your digital goals and make a positive impact in your
                                    success story.
                                </p>
                            </div>
                        </div>
                        <div classa="col-lg-6 col-md-6 col-md-12">
                            <div class="abt-img-block">
                                <img src="assets\images\mix\abt-banner-img.png" class="img-fluid" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>

        <section id="intro-section">
            <div class="ps015">
                <div class="building-overflow-text overflow-text">
                    <marquee width="120%" direction="right" scrollamount="12"><h4>OUR vision OUR vision OUR vision OUR vision OUR vision OUR vision</h4></marquee>
                </div>
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="intro-text-block">
                                <h2>
                                    binaro<br />
                                    <span class="head-style2">vision</span><br />
                                    statement
                                </h2>
                            </div>
                            <div class="intro-desc">
                                <p>
                                    We want to be seen as an infinite provider of cutting-edge digital solutions<br />
                                    that attracts, connects, supports and enables cause-driven individuals and<br />
                                    companies in pursuing their ambitious boundary-moving goals.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-center">
                <div class="heading-style02 fade-in">
                    <div class="heading-style02-img">
                        <img src="assets\images\icons\Vector.png" class="hd-styimg-02 img-fluid" />
                    </div>
                    <a href="jascript:;">in More detail</a>
                </div>
            </div>
        </div>
        <section id="customize-section">
            <div class="ps01">
                <div class="cus-overflow-text overflow-text">
                    <marquee direction="right" scrollamount="12"><h4>OUR MISSION OUR MISSION OUR MISSION OUR MISSION OUR MISSION</h4></marquee>
                </div>
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="intro-text-block">
                                <h2>
                                    Our <br />
                                    <span class="head-style2">mission</span><br />
                                    statement
                                </h2>
                            </div>
                            <div class="intro-desc">
                                <p>
                                    We are here to develop innovative and affordable digital solutions on time while<br />
                                    ensuring exceptional customer service. Through our technically sound business<br />
                                    solutions, we change, simplify and improve the way people live and work. We<br />
                                    commit our infinite mindset and expertise towards creating opportunities and<br />
                                    enabling our stakeholders to achieve their goals.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="golden-section">
            <div class="gray-bg">
                <div class="container">
                    <div class="row justify-content-center align-items-center">
                        <div class="col-lg-6 col-md-6 col-md-12">
                            <div class="heading-style03-img overlap-img-heading">
                                <img src="assets\images\icons\Vector.png" class="hd-styimg-03 img-fluid" />
                            </div>
                            <div class="abt-text-block golden-text-block">
                                <h2>Golden Circle</h2>
                                <p>
                                    Binaro Group is your precise, customized, and economical partner for Design, Development, and Media Services. We are more than just a service provider; we believe in thinking big, being innovative, and
                                    embracing challenges to develop smart and customized solutions that solve problems in unexpected ways. Discover how Binaro Group can help you achieve your digital goals and make a positive impact in your
                                    success story.
                                </p>
                            </div>
                            <div class="golden-btn"><a href="javascript:;">READ MORE</a></div>
                        </div>
                        <div classa="col-lg-6 col-md-6 col-md-12">
                            <div class="abt-img-block">
                                <img src="assets\images\mix\golden-chart.png" class="img-fluid" />
                            </div>
                        </div>
                    </div>
                    <div class="gray-inner-box">
                        <div class="row">
                            <div class="col-lg-3 col-md-3 col-sm-3">
                                <div class="gary-inner-text-block">
                                    <h4>Who?</h4>
                                    <p>We work globally with companies, organizations and institutions that share our way of doing business and engagement towards social responsibility.</p>
                                    <div class="gary-btn-block">
                                        <a href="javascript:;">learn more</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-3 col-sm-3">
                                <div class="gary-inner-text-block">
                                    <h4>Why?</h4>
                                    <p>
                                        We simply love what we do! Our<br />
                                        Binaro family is driven by the feel<br />
                                        and satisfaction of exceeding<br />
                                        expectations and delighting<br />
                                        clients.
                                    </p>
                                    <div class="gary-btn-block">
                                        <a href="javascript:;">learn more</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-3 col-sm-3">
                                <div class="gary-inner-text-block">
                                    <h4>how?</h4>
                                    <p>
                                        We understand that any idea<br />
                                        takes time to evolve and<br />
                                        requirements change throughout<br />
                                        the process. That’s why we invest<br />
                                        what a considerable amount of
                                    </p>
                                    <div class="gary-btn-block">
                                        <a href="javascript:;">learn more</a>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-3 col-md-3 col-sm-3">
                                <div class="gary-inner-text-block">
                                    <h4>what?</h4>
                                    <p>
                                        We are a provider of customized<br />
                                        digital solutions. Our Binaro<br />
                                        Group combines our vast<br />
                                        expertise in several business<br />
                                        fields.
                                    </p>
                                    <div class="gary-btn-block">
                                        <a href="javascript:;">learn more</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section id="our-core">
            <div class="pst80">
                <div class="building-overflow-text overflow-text">
                    <marquee width="120%" direction="right" scrollamount="12"><h4>core values core values core values core values core values core values core values</h4></marquee>
                </div>
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="intro-text-block">
                                <h2>
                                    Our<br />
                                    <span class="head-style2">Core</span><br />
                                    values
                                </h2>
                            </div>
                            <div class="intro-desc">
                                <p>
                                    We are here to develop innovative and affordable digital solutions on time while<br />
                                    ensuring exceptional customer service. Through our technically sound business<br />
                                    solutions, we change, simplify and improve the way people live and work. We commit<br />
                                    our infinite mindset and expertise towards creating opportunities and<br />
                                    enabling our stakeholders to achieve their goals.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 d-flex justify-content-center">
                <div class="heading-style02 fade-in">
                    <div class="heading-style02-img">
                        <img src="assets\images\icons\Vector.png" class="hd-styimg-02 img-fluid" />
                    </div>
                    <a href="jascript:;">in More detail</a>
                </div>
            </div>
        </div>
        <section id="mang-director">
            <div class="bg-blue">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12">
                            <div class="manger-img-box">
                                <img src="assets\images\mix\manager.png" class="hd-styimg-02 img-fluid" />
                            </div>
                            <div class="heading-block">
                                <h1>Konrad Graf</h1>
                                <h2>(Managing Director)</h2>
                                <p>
                                    The last wave of innovation driven by the Internet revolution has radically shaped and transformed<br />
                                    our business world. The momentum of this digital technological shift will continue in the next<br />
                                    decades and will further create drastic economical and sociological changes. My aim is to recognize<br />
                                    current and future trends, to envision and develop affordable and applicable solutions timely, and to<br />
                                    enable businesses to cope with the inevitable digital challenges. Supported by my valued team of <br />
                                    specialists, we work globally with a multitude of companies in laying their digital foundation to <br />
                                    master future market requirements.
                                </p>
                            </div>
                            <div class="social-btn"><a href="javascript:;">Linkedin</a></div>
                        </div>
                    </div>
                    <div class="manger-inner-row">
                        <div class="row">
                            <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="icon-box">
                                    <div class="row">
                                        <div class="col-lg-2 col-md-2 col-sm-12">
                                            <div class="icon-block"><img src="assets\images\icons\head.png" class="hd-styimg-02 img-fluid" /></div>
                                        </div>
                                        <div class="col-lg-10 col-md-10 col-sm-12">
                                            <div class="icon-text-block">
                                                <h4>
                                                    Switzerland Business Studies Higher<br />
                                                    Certificate, Business Administration
                                                </h4>
                                                <p>Kaufmännische Berufsschule Oberengadin, Samedan</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="icon-box">
                                    <div class="row">
                                        <div class="col-lg-2 col-md-2 col-sm-12">
                                            <div class="icon-block"><img src="assets\images\icons\bag.png" class="hd-styimg-02 img-fluid" /></div>
                                        </div>
                                        <div class="col-lg-10 col-md-10 col-sm-12">
                                            <div class="icon-text-block">
                                                <h4>
                                                    Switzerland Business Studies Higher<br />
                                                    Certificate, Business Administration
                                                </h4>
                                                <p>Kaufmännische Berufsschule Oberengadin, Samedan</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row ptop">
                            <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="icon-box">
                                    <div class="row">
                                        <div class="col-lg-2 col-md-2 col-sm-12">
                                            <div class="icon-block"><img src="assets\images\icons\label.png" class="hd-styimg-02 img-fluid" /></div>
                                        </div>
                                        <div class="col-lg-10 col-md-10 col-sm-12">
                                            <div class="icon-text-block">
                                                <h4>
                                                    Switzerland Business Studies Higher<br />
                                                    Certificate, Business Administration
                                                </h4>
                                                <p>Kaufmännische Berufsschule Oberengadin, Samedan</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-6 col-md-6 col-sm-12">
                                <div class="icon-box">
                                    <div class="row">
                                        <div class="col-lg-2 col-md-2 col-sm-12">
                                            <div class="icon-block"><img src="assets\images\icons\label.png" class="hd-styimg-02 img-fluid" /></div>
                                        </div>
                                        <div class="col-lg-10 col-md-10 col-sm-12">
                                            <div class="icon-text-block">
                                                <h4>
                                                    Switzerland Business Studies Higher<br />
                                                    Certificate, Business Administration
                                                </h4>
                                                <p>Kaufmännische Berufsschule Oberengadin, Samedan</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <div class="pad-tp"><?php include("../includes/service_cpt.php") ?></div>
        <?php include("../includes/module-section.php") ?>
        <!-- Footer -->
        <div class="pad-top"><?php include("../includes/footer.php") ?></div>

        <!-- Default Popup -->
        <?php include("../includes/popup.php") ?>

        <!-- Scripts -->
        <?php include("../includes/scripts.php") ?>
    </body>
</html>
